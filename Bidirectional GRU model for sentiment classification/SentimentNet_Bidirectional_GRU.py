...
class SentimentNet_Bidirectional_GRU(nn.Module):
    def __init__(self, vocab_size, output_size, embedding_dim, hidden_dim, n_layers, drop_prob=0.5):
        super(SentimentNet_Bidirectional_GRU, self).__init__()
    ...
    def forward(self, x):
        ...
        # gru_out, _ = self.gru(embeds, (h0, c0))
...
model = SentimentNet_Bidirectional_GRU(vocab_size, output_size, embedding_dim, hidden_dim, n_layers)
model.to(device)
print(model)
...
if Want_To_Train_NN == 1:
    ...
    for i in range(epochs):
        ...
        for inputs, labels in train_loader:
            ...
            optimizer.step()
    torch.save(model.state_dict(), './Network_Q3_P4.pt')
else:
    model.load_state_dict(torch.load('./Network_Q3_P4.pt')) #Loading the model
