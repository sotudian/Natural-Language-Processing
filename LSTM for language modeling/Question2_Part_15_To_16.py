...
tweet_words = ['_PAD','_UNK','</s>'] + tweet_words
...
Do_want_To_Train_Q15 = 0
batch_size = 320
epochs=20
lr=0.001
if Do_want_To_Train_Q15 == 1:
    net15 = WordLSTM_with_Glove() # instantiate the model
    train(net15, epochs, batch_size, lr, 1, 256,tweet_x_int_L5,tweet_y_int_L5) # train the model
    torch.save(net15, 'Q2_Part_15_Network.pt')
else:
    net15 = torch.load('Q2_Part_15_Network.pt')
    net15.eval()
...
for i in range(10):
    print('=======================================')
    print("- Examples "+str(i)+" - LSTM LM - with GloveTweet: ")
    print(sample(net15, size='</s>' , prime="<s>", word2idx_Inp = tweet_word2idx, idx2word_Inp =tweet_idx2word),'\n')


del i,Do_want_To_Train_Q15
...
Testing(net15, batch_size ,Test_2_x_int,Test_2_y_int)     
