...
Test_1_Pride_Text_Equal_seqs = sum([Equal_seq(i,5) for i in Test_1_Preprocessed_Pride_Text], [])

del t,f
# Looking up the mapping dictionary and assigning the index to the respective words
Test_1_Pride_Text_Equal_seqs_INDICES =[]
for i, sentence in enumerate(Test_1_Pride_Text_Equal_seqs):
    Test_1_Pride_Text_Equal_seqs_INDICES.append([tweet_word2idx[word] if word in tweet_word2idx else tweet_word2idx['_UNK'] for word in sentence])
del i, sentence
...
Testing(net8, batch_size ,Test_1_x_int,Test_1_y_int)     
