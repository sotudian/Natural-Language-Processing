...
Pride_Text_Equal_seqs_L25 = sum([Equal_seq(i,25) for i in Preprocessed_Pride_Text], [])

Pride_Text_Equal_seqs_INDICES_L25 =[]
for i, sentence in enumerate(Pride_Text_Equal_seqs_L25):
    Pride_Text_Equal_seqs_INDICES_L25.append([word2idx[word] if word in word2idx else word2idx['_UNK'] for word in sentence])
del i, sentence


X = []
Y = []
for S in Pride_Text_Equal_seqs_INDICES_L25:
  X.append(S[:-1])
  Y.append(S[1:])

x_int_L25 = np.array(X)
y_int_L25 = np.array(Y)
...
if Do_want_To_Train == 1:
    net3 = WordLSTM() # instantiate the model
    net3.cuda() # push the model to GPU
    train(net3, epochs, batch_size, lr, 1, 50,x_int_L25,y_int_L25) # train the model
    torch.save(net3, 'Q2_Part_3_Network.pt')
else:
    net3 = torch.load('Q2_Part_3_Network.pt')
    net3.eval()
...
for i in range(10):
    print('=======================================')
    print("- Examples "+str(i)+" - LSTM LM - L25: ")
    print(sample(net3, size='</s>' , prime="<s>", word2idx_Inp = word2idx, idx2word_Inp =idx2word),'\n')
...
Test_1_Pride_Text_Equal_seqs_L25 = sum([Equal_seq(i,25) for i in Test_1_Preprocessed_Pride_Text], [])


# Looking up the mapping dictionary and assigning the index to the respective words
Test_1_Pride_Text_Equal_seqs_INDICES_L25 =[]
for i, sentence in enumerate(Test_1_Pride_Text_Equal_seqs_L25):
    Test_1_Pride_Text_Equal_seqs_INDICES_L25.append([word2idx[word] if word in word2idx else word2idx['_UNK'] for word in sentence])
del i, sentence


Test_1_X = []
Test_1_Y = []
for S in Test_1_Pride_Text_Equal_seqs_INDICES_L25:
  Test_1_X.append(S[:-1])
  Test_1_Y.append(S[1:])

Test_1_x_int_L25 = np.array(Test_1_X)
Test_1_y_int_L25 = np.array(Test_1_Y)

del Test_1_X,Test_1_Y,S
# Calculate Perplexity
Testing(net3, batch_size ,Test_1_x_int_L25,Test_1_y_int_L25)     
...
Test_2_Pride_Text_Equal_seqs = sum([Equal_seq(i,25) for i in Test_2_Preprocessed_Pride_Text], [])

del t,f
# Looking up the mapping dictionary and assigning the index to the respective words
Test_2_Pride_Text_Equal_seqs_INDICES =[]
for i, sentence in enumerate(Test_2_Pride_Text_Equal_seqs):
    Test_2_Pride_Text_Equal_seqs_INDICES.append([word2idx[word] if word in word2idx else word2idx['_UNK'] for word in sentence])
del i, sentence


Test_2_X = []
Test_2_Y = []
for S in Test_2_Pride_Text_Equal_seqs_INDICES:
  Test_2_X.append(S[:-1])
  Test_2_Y.append(S[1:])
...
# Calculate Perplexity
Testing(net3, batch_size ,Test_2_x_int,Test_2_y_int)     
