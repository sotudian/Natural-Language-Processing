...
tweet_Preprocessed_Text_Equal_seqs_L15 = sum([Equal_seq(i,15) for i in tweet_Preprocessed_Text], [])
# Looking up the mapping dictionary and assigning the index to the respective words
tweet_Preprocessed_Text_Equal_seqs_INDICES_L15 =[]
for i, sentence in enumerate(tweet_Preprocessed_Text_Equal_seqs_L15):
    tweet_Preprocessed_Text_Equal_seqs_INDICES_L15.append([tweet_word2idx[word] if word in tweet_word2idx else tweet_word2idx['_UNK'] for word in sentence])
del i, sentence


X = []
Y = []
for S in tweet_Preprocessed_Text_Equal_seqs_INDICES_L15:
  X.append(S[:-1])
  Y.append(S[1:])

tweet_x_int_L15 = np.array(X)
tweet_y_int_L15 = np.array(Y)
...
Do_want_To_Train_L15 = 0
vocab_size = len(tweet_word2idx)
batch_size = 320
epochs=20
lr=0.001

if Do_want_To_Train_L15 == 1:
    net10 = WordLSTM() # instantiate the model
    net10.cuda() # push the model to GPU
    train(net10, epochs, batch_size, lr, 1, 256,tweet_x_int_L15,tweet_y_int_L15) # train the model
    torch.save(net10, 'Q2_Part_10_Network.pt')
else:
    net10 = torch.load('Q2_Part_10_Network.pt')
    net10.eval()
...
for i in range(10):
    ...
    print(sample(net10, size='</s>' , prime="<s>", word2idx_Inp = tweet_word2idx, idx2word_Inp =tweet_idx2word),'\n')


del X,Y,i,Do_want_To_Train_L15
print('=' * 60)
...
Test_2_Pride_Text_Equal_seqs_L15 = sum([Equal_seq(i,15) for i in Test_2_Preprocessed_Pride_Text], [])

# Looking up the mapping dictionary and assigning the index to the respective words
Test_2_Pride_Text_Equal_seqs_INDICES_L15 =[]
for i, sentence in enumerate(Test_2_Pride_Text_Equal_seqs_L15):
    Test_2_Pride_Text_Equal_seqs_INDICES_L15.append([tweet_word2idx[word] if word in tweet_word2idx else tweet_word2idx['_UNK'] for word in sentence])
del i, sentence


Test_2_X = []
Test_2_Y = []
for S in Test_2_Pride_Text_Equal_seqs_INDICES_L15:
  Test_2_X.append(S[:-1])
  Test_2_Y.append(S[1:])

Test_2_x_int_L15 = np.array(Test_2_X)
Test_2_y_int_L15 = np.array(Test_2_Y)

del Test_2_X,Test_2_Y,S
# Calculate Perplexity
Testing(net10, batch_size ,Test_2_x_int_L15,Test_2_y_int_L15)     
