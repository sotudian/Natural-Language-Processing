...
class SentimentNet_GRU(nn.Module):
    def __init__(self, vocab_size, output_size, embedding_dim, hidden_dim, n_layers, drop_prob=0.5):
        super(SentimentNet_GRU, self).__init__()
...
model = SentimentNet_GRU(vocab_size, output_size, embedding_dim, hidden_dim, n_layers)
model.to(device)
print(model)
...
if Want_To_Train_NN == 1:
    ...
    for i in range(epochs):
        ...
        for inputs, labels in train_loader:
            h = h.data   # Change this: The hidden state for the LSTM is a tuple containing both the cell state and the hidden state, whereas the GRU only has a single hidden state.
            ...
            optimizer.step()
    torch.save(model.state_dict(), './Network_Q3_P2.pt')
else:
    model.load_state_dict(torch.load('./Network_Q3_P2.pt')) #Loading the model
