...
# load the whole embedding glove.twitter into memory
...
words_Q13_Vocab = list(set(words) & set(embeddings_index.keys()))  # Remove words that are not in Glove
words = {k:v for k,v in words.items() if k in words_Q13_Vocab} # all words + removing words are not in GLOVE
del i,sentence,word
...
del i,sentence,Train_Data,Test_Data
...
if Want_To_Train_NN == 1:
    ...
    for i in range(epochs):
        ...
        for inputs, labels in train_loader:
            ...
            optimizer.step()
    torch.save(model.state_dict(), './Network_Q3_P5.pt')
else:
    model.load_state_dict(torch.load('./Network_Q3_P5.pt')) #Loading the model
